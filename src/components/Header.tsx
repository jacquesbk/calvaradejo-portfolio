...
export function Header() {
  ...
  return (
    <header className="z-50 shrink-0 border-b border-[var(--color-border)] bg-[var(--color-surface)]/90 backdrop-blur-md">
      <div className="mx-auto flex max-w-5xl items-center justify-between gap-5 px-4 py-5 sm:px-6 lg:px-8">
        <button
          type="button"
          onClick={() => navigate(SECTION_INDEX.home)}
          className="font-display text-left text-xl font-semibold tracking-tight text-white transition hover:text-[var(--color-accent)]"
        >
          {site.siteName}
        </button>
        ...
        <button
          type="button"
          className="inline-flex items-center justify-center rounded-lg border border-[var(--color-border)] p-2.5 text-white md:hidden"
          aria-expanded={open}
